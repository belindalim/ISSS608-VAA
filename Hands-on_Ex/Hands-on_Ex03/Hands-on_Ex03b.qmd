...
coe_data <- read_csv("data/COEBiddingResultsPrices.csv") %>%
  mutate(
    month = ym(month),
...
# Define plot
p_line <- ggplot(coe_data, aes(x = month, y = premium, color = vehicle_class, group = vehicle_class)) +
  geom_line(size = 1.2) +
  labs(
    title = "COE Premium Over Time — {frame_along}",
    x = "Month", y = "Premium (SGD)", color = "Category"
  ) +
  transition_reveal(month) +
  theme_minimal()

# Animate and save explicitly using gifski_renderer
anim_save(
  filename = "coe_prices_line.gif",
  animation = p_line,
