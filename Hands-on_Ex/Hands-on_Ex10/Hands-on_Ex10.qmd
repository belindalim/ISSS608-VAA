...
con <- dbConnect(odbc::odbc(),
                 .connection_string = "Driver={Microsoft Access Driver (*.mdb, *.accdb)};DBQ=data/Coffee Chain.accdb")
df <- dbReadTable(con, "CoffeeChain Query")
write_rds(df, "data/CoffeeChain.rds")
dbDisconnect(con)
```
...
Check column names of the loaded dataset:
```{r}
colnames(coffeechain)
```
