...
```{r}
pacman::p_load(tidyverse, ggplot2, patchwork, scales, sf, readr, viridis, plotly)
```
...
To compare the life expectancy of males and female for each age group, it is recommended to compute their proportions for comparison by age groups due to the different population numbers in each age group. This helps to standardise the data for equitable comparisons of life expectancy between males and females.

The formula used for computation of males and females proportion respectively is as follows:

**Proportion for each age group = Population in each age group / Total population for each sex**

The illustration for male population proportion are shown below:
...
The code for the Life Expectancy Proportion plot is:
...
combined_plot <- pyramid_plot + life_expectancy_plot +
  plot_layout(ncol = 2, widths = c(1, 1.5), guides = "collect") +
  plot_annotation(
    title = "Age Pyramid and Life Expectancy Comparison",
    theme = theme(
...
Singapore's population age pyramid can be described as a [constrictive pyramid](https://populationeducation.org/what-are-different-types-population-pyramids/), which is typical for countries with higher levels of socio-economic developments. There are low birth and mortality rates, contributing to an ageing population. The largest population groups belong to ages 30-64. As population ages, the proportion of economically productive population will decline.

The population's life expectancy shows that females have longer life expectancy than males especially from age 70 onwards. Notably, the life expectancy of females and males both show sharp decline from age 65, which suggest higher mortality rates for these groups.

With ageing population, it is important to focus on preventive healthcare measures for a healthier population. This enables better quality of life and delayed retirement age for older Singaporeans to contribute economically. Given the lower life expectancy of males, more studies can be done to understand their health factors and morbidity causes to improve their lifespan.
...
final_chart <- pop_plot + pct_plot +
  ...
  plot_annotation(
    title = "Top 20 PAs with highest Elderly Population and Proportion",
    theme = theme(
...
### 3.2.3 Extra visualisation beyond the scope of Take-Home Exercise
...
-   Ministry of Health Singapore. *Action Plan for Successful Ageing*. Retrieved from: https://www.moh.gov.sg/policies-and-legislation/action-plan-for-successful-ageing
