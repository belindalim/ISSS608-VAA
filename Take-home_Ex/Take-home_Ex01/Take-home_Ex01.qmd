...
With this definition, we aim to conduct an exploratory data analysis on the demographic structures and distribution of Singapore in 2024, focusing on the three groups of population, namely
...
```{r}
pacman::p_load(tidyverse, patchwork, sf, readr, ggplot2, viridis, plotly)
```
...
combined_plot <- pyramid_plot + life_expectancy_plot +
  ...
  plot_annotation(title = "Age Pyramid and Life Expectancy Comparison",
                  theme = theme(plot.title = element_text(hjust = 0.5, size = 14))) & 
  theme(legend.position = "bottom", legend.box = "horizontal")  # applies to both plots
combined_plot
```

### 3.1.2 Insights
From the age pyramid
