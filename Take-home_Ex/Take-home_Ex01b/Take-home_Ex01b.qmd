...
Note that my classmate has renamed SZ to Subzone and created two additional columns AG (Age Group) and FA (Functional Age) as shown in my classmate's website (see extract in the Figure below). To ensure that the charts are reproducible, the data pre-processing details and codes should be spelt out.

[Figure 1: Extracts from classmate's website]{.underline}![](images/clipboard-185020519.png)
...
Figure 2: Extracts from my classmate's website.

![](images/clipboard-320653457.png)
...
-   Remove the x-axis label since the information is found in the chart title.

-   Increase the size of the x-axis numbers and title
...
ggplot(data = filtered_data, aes(x = reorder(PA, Pop), y = Pop)) +
  ...
  labs(
    ...
    y = NULL   # Remove y-axis label ("Population")
  ) +
  scale_y_continuous(breaks = c(0, 50000, 100000, 150000, 200000, 250000), labels = comma) +
  theme_minimal(base_size = 16) +
  theme(
    plot.title = element_text(size = 18, face = "bold"),
    axis.text.y = element_text(size = 8),   # Smaller PA names
    axis.text.x = element_text(size = 13),  # Larger x-axis numbers
